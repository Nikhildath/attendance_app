...
import {
  ...
  ChevronLeft, Wallet, CalendarRange, PartyPopper, Banknote, MapPinned, Zap, MessageSquare, Radar, Target, Video
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto p-4 scrollbar-none relative z-10">
        ...
        {roleNav.filter((item) => (item.roles as readonly string[]).includes(profile?.role ?? "Employee")).map((item) => {
          const active = isActive(item.to);
          const Icon = item.icon;
          return (
            <Link
