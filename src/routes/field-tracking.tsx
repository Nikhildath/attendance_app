      ...
      if (profiles) {
        const merged: FieldStaff[] = profiles.map(p => {
          ...
          return {
            ...
            deviceInfo: deviceParts.length ? deviceParts.join(' | ') : undefined,
          } as any;
        });
        setStaff(merged);
        ...
        return prev.map((s) =>
          s.id === data.id
            ? {
                ...
                lastUpdate: new Date(data.lastUpdate).toLocaleTimeString(),
                accuracy: data.accuracy,
                deviceInfo: data.deviceInfo || s.deviceInfo,
              }
            : s
        );
      });
      ...
      setStaff((prev) => {
        const updated = prev.map((s) => {
          ...
          return {
            ...
            deviceInfo: update.deviceInfo || s.deviceInfo,
          };
        });
        ...
        const popupContent = `
          <div class="overflow-hidden">
            ...
            <div class="p-4 space-y-3">
              ...
              ${s.deviceInfo ? `
              <div class="space-y-1">
                <div class="text-[9px] uppercase tracking-wider font-bold text-muted-foreground">Device</div>
                <div class="text-[11px] font-medium">${s.deviceInfo}</div>
              </div>
              ` : ''}
              ...
              <li key={s.id}>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {s.deviceInfo && (
                      <div className="mt-0.5 text-[10px] text-muted-foreground/60 truncate">{s.deviceInfo}</div>
                    )}
                  </div>
                </button>
              </li>
