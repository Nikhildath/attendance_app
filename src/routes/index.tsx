...
function Dashboard() {
  ...
  return (
    <div className="space-y-10 pb-24 md:pb-12 relative overflow-hidden">
      {/* Premium Background Elements */}
      <div className="absolute inset-0 pointer-events-none z-0">
        <div className="absolute -top-[10%] -right-[10%] w-[60%] h-[60%] bg-primary/5 rounded-full blur-[150px] animate-pulse" />
        <div className="absolute top-[20%] -left-[10%] w-[40%] h-[40%] bg-secondary/5 rounded-full blur-[120px]" />
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 relative z-10">
        ...
        >
          <div className="flex items-center gap-4">
             <div className="p-3 bg-white/[0.03] rounded-[1.25rem] border border-white/[0.05] shadow-2xl">
        ...
        <div className="flex items-center gap-3 bg-white/[0.02] backdrop-blur-3xl p-3 rounded-2xl border border-white/[0.05]">
           <div className="flex flex-col items-end">
              <span className="text-[9px] font-black text-white/20 uppercase tracking-widest">Profile Status</span>
        ...
        {announcements.length > 0 && (
          ...
          >
            {announcements.slice(0, 1).map((a) => (
              <div key={a.id} className="group relative overflow-hidden rounded-[2.5rem] p-6 bg-gradient-to-br from-primary/10 to-transparent border border-primary/20 backdrop-blur-3xl shadow-glow">
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                       <h4 className="text-[11px] font-black uppercase tracking-[0.2em] text-primary">{a.title}</h4>
                       <span className="h-1.5 w-1.5 rounded-full bg-primary animate-ping" />
                    </div>
                    <p className="text-sm font-bold text-white/70 mt-1 leading-relaxed line-clamp-1">{a.content}</p>
                  </div>
                </div>
              </div>
            ))}
      ...
      <AnimatePresence>
        {celebration && (
          <motion.div 
            ...
            className="relative overflow-hidden rounded-[3rem] bg-gradient-to-br from-[#6366f1] via-[#a855f7] to-[#ec4899] p-10 text-white shadow-glow relative z-10"
          >
            ...
            <div className="relative flex items-center gap-8">
              ...
              <div>
                ...
                <p className="text-xs font-black opacity-80 uppercase tracking-[0.4em] mt-2">
      ...
      <div className="md:hidden relative z-10 px-2">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/20 italic">Instant Protocols</h3>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10 relative z-10">
        <div className="xl:col-span-8 flex flex-col gap-10">
           ...
           <div className="p-10 rounded-[3.5rem] bg-white/[0.01] backdrop-blur-3xl border border-white/[0.03] shadow-2xl overflow-hidden relative">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
                 <div>
                   ...
                   <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] mt-2">Analytical insights over 7 cycles</p>
                 </div>
                 <div className="flex items-center gap-6 bg-black/20 p-3 rounded-2xl border border-white/5">
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest"><div className="w-2.5 h-2.5 rounded-full bg-primary shadow-glow" /> Present</div>
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest"><div className="w-2.5 h-2.5 rounded-full bg-secondary shadow-glow" /> Late</div>
                 </div>
              </div>
        ...
        <div className="xl:col-span-4 flex flex-col gap-10">
           {/* Quick Stats Card */}
           <div className="p-10 rounded-[3.5rem] bg-[#0a0a0a] border border-white/[0.05] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.8)] flex flex-col gap-8 relative overflow-hidden group">
              ...
              <Link 
                to="/attendance"
                className="w-full py-5 rounded-[2rem] bg-primary text-primary-foreground font-black uppercase tracking-[0.3em] italic flex items-center justify-center gap-4 shadow-glow hover:brightness-110 active:scale-95 transition-all duration-300 group/btn"
              >
           ...
           {/* Recent Activity */}
           <div className="p-10 rounded-[3.5rem] bg-white/[0.01] backdrop-blur-3xl border border-white/[0.03] flex flex-col gap-8 shadow-2xl">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-black italic uppercase tracking-tighter flex items-center gap-3">
              ...
              <div className="space-y-4">
                 {recentActivity.map((a, i) => (
                   <motion.div 
                     ...
                     key={a.id} 
                     className="flex items-center justify-between p-5 rounded-[1.75rem] bg-white/[0.02] border border-white/[0.03] hover:bg-white/[0.05] hover:border-primary/10 transition-all duration-300"
                   >
                      <div className="flex items-center gap-4">
                         <div className={cn(
                           "p-2.5 rounded-xl",
                         ...
                         <div>
                            <p className="text-[11px] font-black uppercase tracking-[0.15em] text-white">{a.action}</p>
                            <p className="text-[9px] font-bold text-white/30 tabular-nums uppercase mt-0.5">{a.time}</p>
                         </div>
                      </div>
        ...
        >
          <div className="flex items-center justify-between mb-8">
            <div>
               ...
               <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em] mt-2 italic">Command & Intelligence Hub</p>
            </div>
            <div className="h-[1px] flex-1 bg-white/5 mx-10 hidden md:block" />
...
function QuickActionLink({ to, icon: Icon, label, color }: any) {
  return (
    <Link to={to} className="flex flex-col items-center gap-3 min-w-[80px] group">
      <div className={cn(
        "flex h-16 w-16 items-center justify-center rounded-[1.5rem] border border-white/5 bg-white/[0.02] backdrop-blur-3xl shadow-2xl transition-all duration-500",
        "group-hover:border-primary/30 group-hover:bg-primary/5 group-active:scale-90",
        color
      )}>
        <Icon className="h-7 w-7" />
      </div>
      <span className="text-[9px] font-black uppercase tracking-widest text-white/40 group-hover:text-white transition-colors">{label}</span>
    </Link>
  );
}
...
function MetricCard({ icon: Icon, label, value, sub, color }: any) {
  ...
  return (
    <motion.div 
      ...
      className={cn("p-8 rounded-[3rem] border backdrop-blur-3xl flex flex-col gap-6 transition-all duration-500", colors[color])}
    >
      <div className="flex items-center justify-between">
      ...
      <div>
        <p className="text-4xl font-black italic tracking-tighter tabular-nums text-white">{value}</p>
...
function QuickItem({ label, value, icon: Icon, color }: any) {
  return (
    <div className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.03] flex items-center justify-between group-hover:bg-white/[0.04] transition-colors">
       <div className="flex items-center gap-4">
