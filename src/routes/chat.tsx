...
import { 
  ...
  Paperclip, ChevronLeft, Mic, Square, Play, Pause,
...
function ChatPage() {
  ...
  const typingEmitRef = useRef<boolean>(false);
    ...
    const initChat = async () => {
      ...
      try {
        ...
        if (!socketService.isConnected()) {
          socketService.connect(SOCKET_URL, '', stableId).catch((err) => {
  ...
  useEffect(() => {
    ...
    socketService.joinChatRoom(activeRoom.id);
    setTypingUsers([]);
    ...
    // Keep Supabase Realtime as fallback for reliability
    ...
    return () => { 
      socketService.leaveChatRoom(activeRoom.id);
      unsubMsg();
      unsubTyping();
  ...
  const sendMessage = async (content: string, type: Message['type'] = 'text', fileUrl?: string) => {
    ...
    const { data: newMsg, error } = await chatSupabase
      .from('messages')
      .insert([{
          ...
          <>
            ...
            <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-4 lg:p-6 scrollbar-none space-y-6 lg:space-y-8">
              ...
              {typingUsers.length > 0 && (
                <div className="flex items-center gap-2 px-1 py-2 text-xs text-muted-foreground animate-in fade-in duration-200">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-medium">{typingUsers.map(u => u.username).join(', ')} typing...</span>
                  </div>
                </div>
              )}
                ...
                <form onSubmit={(e) => { e.preventDefault(); if(input.trim()) sendMessage(input); }} className="flex flex-1 items-center gap-2 lg:gap-4">
                  <input 
                    ...
                    onChange={(e) => {
                      ...
                      if (!typingEmitRef.current && profile && activeRoom) {
                        ...
                        setTimeout(() => { typingEmitRef.current = false; }, 2000);
                      }
