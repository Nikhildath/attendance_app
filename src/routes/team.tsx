  ...
  return (
    <div className="space-y-10">
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-3 bg-white/[0.03] backdrop-blur-3xl px-5 py-2.5 rounded-2xl border border-white/[0.05] shadow-2xl">
            <Users className="h-5 w-5 text-primary" />
            <span className="text-xl font-black italic tracking-tighter">{members.length}</span>
            <span className="text-[10px] font-black uppercase tracking-widest text-white/30">Personnel</span>
          </div>
        }
      />
      ...
      <div className="grid gap-10 lg:grid-cols-[1.6fr_1fr]">
        <div className="flex flex-col gap-6">
          <div className="p-8 rounded-[3rem] bg-white/[0.01] backdrop-blur-3xl border border-white/[0.03] shadow-2xl overflow-hidden relative">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
              ...
              <div className="relative max-w-xs w-full">
                ...
                <input 
                  ...
                  className="w-full h-10 bg-white/[0.02] border border-white/5 rounded-xl pl-9 text-[10px] font-black uppercase tracking-widest focus:border-primary/40 outline-none"
                />
              </div>
            </div>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-white/[0.03] text-left text-[10px] font-black uppercase tracking-[0.2em] text-white/30">
                  <tr>
                ...
                <tbody className="divide-y divide-white/[0.03]">
                  {loading ? (
                     <tr><td colSpan={5} className="py-20 text-center text-[10px] font-black uppercase tracking-widest text-white/10">Synchronizing Data...</td></tr>
                    ...
                    <tr key={m.id} className="group hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-white/30">{m.role}</td>
            ...
            <div className="grid grid-cols-1 gap-4 md:hidden">
              {loading ? (
                <div className="py-20 text-center text-[10px] font-black uppercase tracking-widest text-white/10">Synchronizing Data...</div>
              ) : members.filter(m => !q || m.name.toLowerCase().includes(q.toLowerCase())).map((m) => (
                <div key={m.id} className="p-6 rounded-3xl bg-white/[0.02] border border-white/[0.03] space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <Avatar2D name={m.name} size={44} src={m.avatar_url} />
                      <div>
                        <h4 className="font-bold text-sm tracking-tight">{m.name}</h4>
                        <p className="text-[9px] font-black uppercase tracking-widest text-white/30">{m.role}</p>
                      </div>
                    </div>
                    <StatusBadge status={m.currentStatus} />
                  </div>
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t border-white/5">
                    <div>
                      <p className="text-[8px] font-black uppercase tracking-[0.2em] text-white/20">Check-In</p>
                      <p className="text-xs font-black italic text-success mt-1 tabular-nums">{m.checkIn}</p>
                    </div>
                    <div>
                      <p className="text-[8px] font-black uppercase tracking-[0.2em] text-white/20">Check-Out</p>
        ...
        <div className="flex flex-col gap-6">
          <div className="p-10 rounded-[3rem] bg-[#0a0a0a] border border-white/[0.05] shadow-2xl flex flex-col gap-8 h-full relative overflow-hidden group">
            ...
            <div className="flex flex-col gap-2 relative z-10">
              ...
              <p className="text-[9px] font-black text-white/20 uppercase tracking-[0.3em]">Pending operational authorization</p>
            </div>
            ...
            <div className="space-y-4 relative z-10">
              {requests.length === 0 ? (
                ...
                <motion.div 
                  ...
                  key={r.id} 
                  className="p-6 rounded-[2rem] bg-white/[0.02] border border-white/[0.03] hover:bg-white/[0.04] transition-all"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="text-sm font-bold text-white tracking-tight">{r.profiles?.name}</div>
                      <div className="text-[9px] font-black uppercase tracking-widest text-primary mt-1">{r.type} · {r.days} Days</div>
                      <div className="mt-3 text-xs text-white/40 leading-relaxed italic">"{r.reason}"</div>
                    </div>
                  ...
                  {r.status === "Pending" && (
                    <div className="mt-6 flex gap-3">
                      ...
                      <button
                        ...
                        className="flex-1 h-10 rounded-xl bg-white/5 border border-white/5 text-white/40 text-[10px] font-black uppercase tracking-widest hover:bg-secondary/20 hover:text-secondary hover:border-secondary/20 active:scale-95 transition-all"
                      >
                        Reject
                      </button>
                    </div>
            ...
            <button className="mt-auto group flex items-center justify-center gap-4 p-5 rounded-[2rem] bg-white/[0.02] border border-white/[0.05] text-white/30 font-black uppercase tracking-[0.3em] text-[10px] hover:bg-primary/5 hover:text-primary transition-all">
